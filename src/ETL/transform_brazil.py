...
class TransformCovidBrazil:
    ...
    def transform(self, df):
        ...
        # evitar diffs negativos (ajuste simples)
        df['casos_diarios'] = df['casos_diarios'].apply(lambda x: x if x >= 0 else 0)
        df['mortes_diarias'] = df['mortes_diarias'].apply(lambda x: x if x >= 0 else 0)
        df['recuperados_diarios'] = df['recuperados_diarios'].apply(lambda x: x if x >= 0 else 0)
